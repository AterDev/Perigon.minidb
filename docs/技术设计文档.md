...
```csharp
// 创建上下文（自动共享内存缓存）
await using var db = new MicroDbContext("data.mdb");
...
```csharp
// 当确实需要释放内存时（例如应用关闭前）
await MicroDbContext.ReleaseSharedCacheAsync("data.mdb");
// 或同步版本
MicroDbContext.ReleaseSharedCache("data.mdb");
```
...
// 使用（异步推荐）
await using var ctx = new MyDbContext("data.mdb");
...
```csharp
// 创建多个上下文（共享内存）
await using var db1 = new MyDbContext("data.mdb");
await using var db2 = new MyDbContext("data.mdb");
// db1 和 db2 看到的是同一份内存数据

// 显式释放内存（应用关闭前）
await MyDbContext.ReleaseSharedCacheAsync("data.mdb");
// 或同步版本
MyDbContext.ReleaseSharedCache("data.mdb");
```
